...
# Chromium for resume HTML → A4 PDF (Playwright)
RUN npx playwright install --with-deps chromium
RUN npm run build
RUN node -e 'const fs=require("fs"); const pkgs=["mongoose","geoip-lite","cloudinary","nodemailer","google-play-scraper","googleapis","firebase-admin","pdf-lib","playwright"]; for (const p of pkgs) { if (!fs.existsSync(".next/standalone/node_modules/"+p)) throw new Error("standalone missing "+p); } if (!fs.existsSync(".next/standalone/node_modules/geoip-lite/data/geoip-city.dat")) throw new Error("geoip-lite data missing from standalone");'
